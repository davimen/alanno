...
add_action('template_redirect', 'ib2_internal_conversion', 99);
	...
	if ( is_array($meta) ) {
		...
		foreach ( $meta as $post_id ) {
			...
			if ( $last_session ) {
				...
				if ( $diff > 3600 ) // visit conversion page an hour after a session start (a session last for 30 min)...
					continue;
				
				if ( $diff <= 3600 ) {
					// check if a conversion rate has been record for this session...
					$last_conversion = strtotime($old->date);
					$conv_diff = $last_conversion - $last_session;
					if ( $conv_diff > 0 )
						continue;
				}
			}
			...
			if ( empty($referer) && $old ) {
				// Do NOT record conversion because visitors come from direct traffic and a conversion has been recorded for this visitor...
				continue;
			}
