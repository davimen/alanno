<?php
	...
	class DBFUNCTION {
			...
			function doSQL($sql) {
				...
				try {
						$this->dbConnect();
						$rst=mysql_query($sql) or die("Query has error");
						return $rst;
				}catch (Exception $ex) {
...
function processLogin($username,$password){
	...
	if($this->totalRows($result)>0) {
            ...
            $thoigianglogin = date("d-m-Y h:i:s",time());
            $online = "Online";
            $this->updateTable('webmaster',array('online'=>$online,"thoigianglogin"=>$thoigianglogin),"id=".$_SESSION["user_login"]["id"]."");
