	...
	class BUSINESSLOGIC extends DBFUNCTION {
            ...
            function Footer() {
                    ...
                    <div style='width:100%;margin:0 auto;background-color:#4f81b3;overflow:hidden'>
                        ...
                        <div class='clear'></div>
                        <div class='footer'>
                            <div class='left'>Bản quyền © 2017 alona.vn</div>
                    ...
                    if(!empty($value))
                    {
                        ...
                        if(count($array[0])>0)
                        {
                         ...
                         foreach($array[0] as $k=>$v)
                          {
                            $content.="<td><b>".$k."</b>:".$v."</td>";
                          }
                          $content.="</tr></table>";

                            $arraytmp=$GLOBALS["arrayLog"];
      	                    $arraytmp["action"]="Delete";
                            $arraytmp["user"]=$_SESSION["user_login"]["username"];
                            $arraytmp["deleting"]=2;
                            $arraytmp["content"]=$content;
                            $arraytmp["tablename"]=$tableName;
                            $arraytmp["capnhatngay"]=date("d-m-Y h:i:s",time());
            ...
            function logInsert($tableName,$colInsert)
            {
                ...
                $arraytmp["deleting"]=3;
                $arraytmp["user"]=$_SESSION["user_login"]["username"];
                $arraytmp["description"]=$colInsert["description"];
                $arraytmp["content"]=$colInsert["content"];
                $arraytmp["tablename"]=$tableName;
                $arraytmp["capnhatngay"]=date("d-m-Y h:i:s",time());
    	        $this->writeLog($arraytmp);
            }
            ...
            function checkProcode($pro_code,$table){
               $pro_code= strtoupper($pro_code);
               $result = $this->getDynamic($table,"pro_code='".$pro_code."'","");
               $total_res = $this->totalRows($result);

               if($total_res>0)
               {
                    $tam_total = $total_res + 1;
                    $pro_code_tam = $pro_code.$tam_total;
                    return $this->checkProcode($pro_code_tam,$table);
               }
               else
               {
                 return $pro_code;
            ...
            function checkProcodeUpdate($pro_code,$table,$id){
              //echo "sdfjsdf".$id;
              //exit();
               $pro_code= strtoupper($pro_code);
               ...
               if($total_id>0)
               {
                   ...
                   $pro_code_id = $rows["pro_code"];
                   //exit();
                   //echo $rewrite."<br/>";
                   //echo $title_rewrite_id."<br/>";
                   //exit();
                   if($pro_code==$pro_code_id)
                   {
                       return $pro_code;
                   }else
                   {
                       return $this->checkProcode($pro_code,$table);
                   }
               }
			...
			function insert($tableName,&$arrayCol)
            {
                ...
                if($tableName=='article')
                {
                    ...
                    if($pro_code!='')
                    {
                      $colInsert["pro_code"]=$this->checkProcode($pro_code,$tableName);
                    }
                }
                ...
                if($tableName=='article' || $tableName=='article_category' || $tableName=='article_nhasanxuat' )
                {
                    ...
                    $colInsert["title_rewrite"]=$this->checkRewite($colInsert["title_rewrite"],$tableName);
                ...
                */
            	...
            	if($affected>0)
                {
                    if($tableName!="history_log")
                    {
                        $this->logInsert($tableName,$colInsert);
                    }
  		            $js->displayJS("huy();");
			...
			function generateControls_main($arrayConst){
                    ...
                    foreach($Meta['Field'] as $field) {
                                  ...
                                  // javascript
                                    ...
                                    if($_GET["table_name"]=='article')
                                    {
                                        ...
                                        //echo "kq:".strstr('1,2,3,4,5',3)."<br/>";
                                        //echo "type_id:".$field['type_id']."<br/>";
                                        //echo "type_article:".$type_article."<br/>";
				...
				<tr>
				  ...
				  <tr>
					  <td class='boxGrey'>".$Label."</td>
					  <td class='boxGrey2'>
					  <input name='".$Field."' id='".$Field."' type='text' ".$this->focusText()." class='nd2' value=\"".$arrayValue[$Field]."\" /> Ex: http://www.angiang24h.com
					  </td>
				  </tr>";
														}
			...
			function generateControls(&$fullColumn,&$excludeCol,&$col,&$arrayValue,$isInsert,$isEdit,$bienTemplate="",$arrayConst) 			{
				...
				<tr>
				  ...
				  <tr>
					  <td class='boxGrey'>".$fName."</td>
					  <td class='boxGrey2'>
					  <input name='".$value."' id='".$value."' type='text' ".$this->focusText()." class='nd2' value=\"".$arrayValue[$value]."\" /> Ex: http://www.angiang24h.com
					  </td>
				  </tr>";
														}
            ...
            function generateArrayControls(&$fullColumn,&$excludeCol,&$col,&$arrayValue,$arrayid,$isInsert,$isEdit,$bienTemplate="",$arrayConst) {
                ...
                <tr>
                    ...
                    <td class='boxGrey2'>".$html->input("fields[".$array_value."][".$value."][]",array('class' => 'nd','type' => 'text','value' => $arrayValue[$array_value][$value],'onfocus' => 'fo(this)','onblur' => 'lo(this)','maxlength' => '12','onkeypress' => "return nhapso(event,'".$value."')"))." ".$symbol."</td>
                                                  ...
                                                  <td class='boxGrey'>".$fName."</td>
                                                  <td class='boxGrey2'>
                                                   <input name='fields[".$array_value."][".$value."][]' id='fields[".$array_value."][".$value."][]' type='text' ".$this->focusText()." class='nd2' value=\"".$arrayValue[$array_value][$value]."\" /> Ex: http://www.angiang24h.com
                                                  </td>
                                    </tr>";
																	}
																	elseif(strstr($value,"password")!="") {
    ...
    <tr>
          <td class='boxGrey2' colspan='2'>
			...
			*******************************************************************/
            ...
            {
              if(count($arrayLog)>0)
              return $this->insertTable("history_log",$arrayLog);
			}
			/*
			Paging on one table
			*******************************************************************/
           ...
           function invoice_order($Param){
               $row = $Param['row'];
               $str="";
               $str="<a class='invoice_order' onClick='openBox(&quot;invoice_order.php?order_id=".$row["id"]."&quot;,900,700);' href='javascript:void(0)' style='width:70px; float:left'><span>Chi tiết</span></a>";
               return $str;
            }
			...
			function task_notify($arrData){
				...
				$paramater=array(	"EmailFrom"=>"contact@vlink.vn",
									"FromName"=>"VLINK MANAGEMENT",
									"EmailTo"=>$EmailTo,
									"ToName"=>"",
									"AddCC"=>"thuyentc@vlink.vn",
									"AddBCC"=>"",
									"Subject"=>$arrData['title'],
									"Content"=>$arrData['description'].$CONFIG['EMAIL_SIGNATURE']
								);
