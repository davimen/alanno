...
if(!$isEdit && !$isInsert){ /* Gridview ****************************************/
	...
	foreach($arrayConst['Meta']['Field'] as $Field => $FieldInfo){
		if($FieldInfo['isFilter']){
			if($FieldInfo['Type']!='int'){
			    $cbo=$dbf->textFilter($Field,$arrayConst["filter_parameter"][$Field],$FieldInfo['Label'].' ',array("text"=>"Select","onchange"=>"updateCategory2(this,$('#arrayid').attr('value'));","firstText"=>"Select"));
    ...
    {
        if(isset($_GET["article_category_id"]) && $_GET["article_category_id"]!="" || isset($_GET["discount"])  || isset($_GET["pro_code"]) || isset($_GET["pro_end"]))
        {
